...
# 原理简述：提取每个样本的特征向量；用 k 近邻方法看该样本周围是不是都属于同一类；如果不是，就说明这张图像可能被标错。
#使用 sklearn.neighbors.NearestNeighbors 计算近邻；提供了一系列辅助函数：相似度矩阵、KNN 分布、得分计算等
...
def noniterate_detection(config, record, train_dataset, sel_noisy=[]):
    ...
    if config.method == 'rank1':
        # ================== 关键改动开始 ==================
        # 用类别数 KINDS 作为 key，为每个 KINDS 维护一套 T_init / p_init
        KINDS = int(config.num_classes)

        # 取出当前 KINDS 对应的子字典，如果没有就新建一个空的
        sub_dic = global_dic.get(KINDS, {})

        # 从子字典里取 T_init / p_init
        T_init = sub_dic.get('T_init', None)
        p_init = sub_dic.get('p_init', None)

        # 调用 get_T_global_min_new，用的是这个 KINDS 的 sub_dic
        T, p, sub_dic = get_T_global_min_new(
            config, data_set=data_set,
            max_step=config.max_iter if T_init is None else 20,
            lr=0.1 if T_init is None else 0.01,
            NumTest=config.G, T0=T_init, p0=p_init,
            global_dic=sub_dic
        )

        # 更新回总的 global_dic：只更新当前 KINDS 这一格
        global_dic[KINDS] = sub_dic
        # ================== 关键改动结束 ==================
    ...
    sel_noisy = get_knn_acc_all_class(
        config, data_set, k=config.k, noise_prior=noisy_prior,
        sel_noisy=sel_noisy, thre_noise_rate=T_given_noisy,
        thre_true=T_given_noisy_true
    )
