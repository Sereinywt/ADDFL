...
import time
import warnings
import math # 新增：用于 Manual_iteration 方法中的 math.ceil
...
class DfauLo():
    ...
    import os, re, glob
    #新增
    def _rekey_name2isfault(self, n2f: dict, train_root: str) -> dict:
        """
        将 name2isfault 的键统一为真实 basename。
        若检测到键形如 '^\d+\.png$', 则按文件名排序对齐：i.png -> 第 i 个文件的 basename。
        :param n2f: 原始 name2isfault dict
        :param train_root: 例如 ./dataset/RandomDataNoise/MNIST/train
        """
        # 先把磁盘上所有 png 文件名拉出来（两层 class/xxx.png）
        files = []
        for p in glob.glob(os.path.join(train_root, '*', '*')):
            if p.lower().endswith('.png') and os.path.isfile(p):
                files.append(os.path.basename(p))
        files.sort()  # 确定顺序

        # 情况 A：键本来就是文件名或包含路径 -> 直接规范成 basename
        if not n2f or not all(re.match(r'^\d+\.png$', k) for k in n2f.keys()):
            return {os.path.basename(k): bool(v) for k, v in n2f.items()}

        # 情况 B：全是数字键 'i.png' -> 映射到按文件名排序后的第 i 个
        if len(files) != len(n2f):
            raise RuntimeError(f"name2isfault 序号键数量({len(n2f)})与磁盘文件数({len(files)})不一致，无法一一对齐。")

        mapped = {}
        for i, fname in enumerate(files):
            k = f"{i}.png"
            if k not in n2f:
                raise KeyError(f"name2isfault 缺少键：{k}")
            mapped[fname] = bool(n2f[k])
        return mapped
    ...
    def run(self, data_s):
        ...
        if os.path.exists(
            ...
            # === 保存 noManual 结果（在进入 manual 之前就写死，避免被覆盖）===
            out_dir = os.path.join(self.args.dataset, 'results', self.args.model_name)
            os.makedirs(out_dir, exist_ok=True)

            nm_list_path  = os.path.join(out_dir, 'noManual_results_list.json')
            nm_score_path = os.path.join(out_dir, 'noManual_sorted_score_list.json')

            print("[SAVE] noManual counts:",
                  len(noManual_results_list) if 'noManual_results_list' in locals() else 'NA',
                  len(noManual_sorted_score_list) if 'noManual_sorted_score_list' in locals() else 'NA')

            # 原子写入，避免被中途截断
            import json, os, tempfile, shutil
            for _path, _obj in [(nm_list_path, noManual_results_list),
                                (nm_score_path, noManual_sorted_score_list)]:
                tmp = _path + ".tmp"
                with open(tmp, "w") as f:
                    json.dump(_obj, f, indent=2)
  ...
  #      return data
    def load_json(self, load_path):
        # === 新增：自动生成 random_index.json ===
        if not os.path.exists(load_path):
            if os.path.basename(load_path) == "random_index.json":
                print(f"[WARN] {load_path} missing, creating default random_index.json ...")
                base_dir = os.path.dirname(load_path)
                gt_path = os.path.join(base_dir, "gt_list.json")
                if os.path.exists(gt_path):
                    import numpy as np
                    gt = json.load(open(gt_path))
                    cls = np.unique(gt)
                    random_index = [int(np.where(np.array(gt)==c)[0][0]) for c in cls]
                    with open(load_path, "w") as f:
                        json.dump(random_index, f, indent=4)
                    print(f"✅ Created {load_path} with {len(random_index)} entries.")
                else:
                    raise FileNotFoundError(f"Cannot auto-create random_index.json because {gt_path} is missing.")
            else:
                raise FileNotFoundError(f"Missing required file: {load_path}")
        # === 原逻辑 ===
        with open(load_path, 'r') as f:
            return json.load(f)
...
DataSet    = 'MNIST'
NoiseType  = 'RandomDataNoise'      # 或其它四类之一
Model      = 'LeNet5'               # 也可 'LeNet1'
hook_layer = 'c5'                   # LeNet5 用 'c5'，LeNet1 用 'c3'
image_size = '(28, 28, 1)'
random.seed(2023)
