...
from utils.models import ResNet18_CIFAR, ResNet18_EMNIST
...
class Baselines():
    ...
    def _get_dataset_name_upper(self):
        """
        推断数据集名：
        1) 如果 args 有 dataset_name 字段，就直接用它
        2) 否则从 args.dataset 这个路径的最后一段取名字
           例如: ./dataset/RandomLabelNoise/CIFAR10 -> CIFAR10
        """
        # 缓存一下，避免每次重新解析
        if hasattr(self, "_dataset_name_upper") and self._dataset_name_upper is not None:
            return self._dataset_name_upper

        ds = ""

        # 情况 1：有 dataset_name
        if hasattr(self.args, "dataset_name") and self.args.dataset_name is not None:
            ds = str(self.args.dataset_name)
        # 情况 2：只有 dataset 路径
        elif hasattr(self.args, "dataset"):
            path = str(self.args.dataset).replace("\\", "/")
            parts = [p for p in path.split("/") if p]
            if parts:
                ds = parts[-1]   # 例如 'CIFAR10'

        self._dataset_name_upper = ds.upper()
        return self._dataset_name_upper

    def _resolve_model_class(self):
        ds = self._get_dataset_name_upper()
        name = self.args.model_name

        if name == "ResNet18":
            if ds == "CIFAR10":
                return ResNet18_CIFAR
            elif ds in ("MNIST", "KMNIST", "EMNIST"):
                return ResNet18_EMNIST
            else:
                raise ValueError(f"No ResNet18 variant for dataset {ds}")
    ...
    def DIF(self, data_s):  #DIF()：Deep Isolation Forest，检测异常样本
        DIF_time = -1
        if os.path.exists(
    ...
    def SimiFeat(self, data_s):  #SimiFeat()：基于特征聚类的可疑样本识别（用了 simifeat.py 中的核心逻辑）
        SimiFeat_time = -1
        if os.path.exists(os.path.join(self.args.dataset,
            ...
            # 尝试把 num_classes 传进去（LeNet5、ResNet18_* 都支持这个参数）
            try:
                model = model_class(num_classes=class_num)
            except TypeError:
                # 某些模型构造函数可能没有 num_classes 参数，就退回无参数版本
                model = model_class()
...
#                 print("Difference of two approaches: ", diff)
    ...
    def NCNV(self, data_s):  #NCNV()：自己训练两个网络，比较预测一致性找异常
        NCNV_time = -1
        if os.path.exists(os.path.join(self.args.dataset,
            ...
            classes = self.load_json(self.args.class_path)
            class_num = len(classes.keys())
            # 根据当前 model_name 找到对应的 *_NCNV 类
            model_ncnv_class = eval(self.args.model_name + '_NCNV')
            # ⭐ 按类别数构造 NCNV 模型（LeNet5_NCNV / ResNet18_NCNV 都支持 num_classes）
            try:
                model1 = model_ncnv_class(num_classes=class_num)
                model2 = model_ncnv_class(num_classes=class_num)
            except TypeError:
                # 万一有别的 *_NCNV 没有 num_classes 参数，就退回老的无参写法
                model1 = model_ncnv_class()
                model2 = model_ncnv_class()
            ...
            for epoch in range(epoches):
                ...
                if epoch < warmup:
                    for i, (images, labels, image_paths) in enumerate(data_loader):
                        if i == 0:   # 只打印一次即可，避免刷屏
                            print("NCNV 输入图像 shape =", images.shape)
                            print("[NCNV][DEBUG] labels min/max =",
                                  labels.min().item(), labels.max().item())
                        out1, fea1 = model1(images.to(device), feat=True)
                        out2 = model2(images.to(device))
                        if i == 0 and epoch == 0:
                            print("[NCNV][DEBUG] out1.shape =", out1.shape,
                                  "  class_num =", class_num)
    ...
    def DeepGini(self,data_s):
        if os.path.exists(os.path.join(self.args.dataset,
            ...
            # simulate get feature process (actually, we have already got the feature, but we need to calculate the total time)
            modelargs = torch.load(self.args.model_args)
            # === 新增：根据 class_path 计算类别数 ===
            with open(self.args.class_path, 'r', encoding='utf-8') as f:
                class_map = json.load(f)   # 比如 {"Aa": 0, "Bb": 1, ...}
            num_classes = len(class_map)   # EMNIST 就是 26，MNIST/CIFAR10 就是 10
            dataset_ = dataset(root=self.args.dataset, classes_path=self.args.class_path,
                               transform=modelargs['transform'],
                               image_size=eval(self.args.image_size), image_set=self.args.image_set,
                               specific_label=None, data_s=data_s)
    ...
    def CleanLab(self,data_s):
        if os.path.exists(os.path.join(self.args.dataset,
            ...
            with open(self.args.class_path, 'r', encoding='utf-8') as f:
                class_map = json.load(f)      # 比如 {"Aa": 0, "Bb": 1, ...}
            num_classes = len(class_map)      # EMNIST=26，MNIST/CIFAR10=10
            ...
            dataset_ = dataset(root=self.args.dataset, classes_path=self.args.class_path,
                               ...
                               specific_label=None, data_s=data_s)
            data_loader = torch.utils.data.DataLoader(dataset_, batch_size=1, shuffle=False, num_workers=0)
            # 用和 DeepGini 一样的方式解析模型类
            model_class = self._resolve_model_class()
            try:
                model = model_class(num_classes=num_classes)
            except TypeError:
                # 以防有些模型构造函数没有 num_classes 这个参数
                model = model_class()
            state_dict = torch.load(self.args.model)
            model.load_state_dict(state_dict)
            loss_fn = modelargs['loss_fn']
